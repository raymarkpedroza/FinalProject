...
namespace PastebookBusinessLayer.BusinessLayer
{
    public class PostManager : IPostManager
    {
        ...
        public List<PASTEBOOK_POST> RetrieveNewsfeedPost(int id, List<int> listOfFriendIds)
        {
            ...
            listOfUsersPost = _postRepo.RetrieveList(x => x.POSTER_ID.Equals(id), poster => poster.PASTEBOOK_USER, profileOwner => profileOwner.PASTEBOOK_USER1, comments => comments.PASTEBOOK_COMMENT.Select(commentator => commentator.PASTEBOOK_USER), likes => likes.PASTEBOOK_LIKE.Select(liker => likes.PASTEBOOK_USER));
        ...
        public PASTEBOOK_POST RetrievePost(int postId)
        {
            ...
            post = _postRepo.RetrieveSpecificRecord(x=>x.ID.Equals(postId));
